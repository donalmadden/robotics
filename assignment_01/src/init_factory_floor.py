...
from assignment_01.srv import SpawnTurtles, SpawnTurtlesResponse
...
def my_callback(request):
    rospy.loginfo("Service /init_factory_floor has been called " + str(request.numTurtles))
    ...
    rospy.loginfo("Finished service /init_factory_floor")
    resp = SpawnTurtlesResponse()
    resp.coordinates = _created_turtle_locations
    resp.names = _created_turtle_names
    return resp


rospy.init_node('init_factory_floor_service_server')
rate = rospy.Rate(1)
spawn_turtle = rospy.ServiceProxy('spawn', Spawn)
_ = rospy.Service('/init_factory_floor', SpawnTurtles, my_callback)
